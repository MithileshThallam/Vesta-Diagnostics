...
export const TestManagement = () => {
  ...
  const createTestHandler = useCallback(async (newTest: Omit<MedicalTest, "id">) => {
    try {
      ...
      const response = await fetch("https://vesta-diagnostics-t4nn.vercel.app/api/tests/create", {
        method: "POST",
  ...
  const editTestHandler = useCallback(async (updatedTest: MedicalTest | Omit<MedicalTest, "id">) => {
    ...
    try {
      ...
      const res = await fetch(`https://vesta-diagnostics-t4nn.vercel.app/api/tests/${updatedTest.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify(testData),
